...
export const mergeExtraArtistsData = async (master: ReleaseDb, releases: Release[]) => {
...
export const discogs_getAllReleasesByMasterId = async (masterId: number) => {
  ...
  const releases = await client.database().getMasterVersions(masterId, { page: 1, per_page: 10 })
